...
export class HudWindow extends WindowBase {
    ...
    constructor() {
        ...
        browserModule.on("Client:Hud:CloseNativeMenu", () => (this.nativeMenu = false));
        browserModule.on("Client:Hud:PushNotification", this.pushNotification.bind(this));
        browserModule.on("Client:Hud:NativeMenu:ToggleGameControls", (state: boolean) => alt.toggleGameControls(!state));
    ...
    private vehicleTick(): void {
        ...
        browserModule.call("Hud:Vehicle:Update", Math.round(veh.speed * 3.6), rpm);
    }
    ...
    private onKeyDown(key: number): void {
        if (browserModule.isAnyComponentActive("Death")) return;
    ...
    public onShow(state: boolean, data: string): void {
        ...
        browserModule.call("Hud:ShowVoice", state, 0, 0);
    }
}
