...
class NMenuWindow extends WindowBase {
  ...
  private keydown(key: number): void {
    if(!this._canOpen || browserModule.isAnyComponentActive('NMenu') || !playerModule.alive || alt.Player.local.getStreamSyncedMeta('ROPED') || alt.Player.local.getStreamSyncedMeta('CUFFED') || key != KeyCode.KEY_N) return;
