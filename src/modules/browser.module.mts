...
export default new (class BrowserModule extends ModuleBase {
    ...
    public isAnyComponentActive(...exclude: string[]): boolean {
        ...
        return this._windows.find((x) => x.visible && !exclude.includes(x.name)) != null || (!exclude.includes("Chat") && hud.chatActive) || (!exclude.includes("Progressbar") && hud.progressbar) || (!exclude.includes("NativeMenu") && hud.nativeMenu) || alt.isConsoleOpen() || game.isPauseMenuActive();
    }
